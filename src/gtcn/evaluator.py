...
def predict_sequence(model: GTCNModel, sequence: DSequence, batch_size: int = 64) -> Dict[int, GestureLabel]:
    """
    ...
    if len(windows) == 0:
        print(f"Warning: Sequence {sequence.sequence_id} has fewer frames than window length")
        return {}
...
def merge_consecutive_gestures(frame_predictions: Dict[int, GestureLabel]) -> List[Tuple[GestureLabel, int, int]]:
    """
...
def filter_short_gestures(gestures: List[Tuple[GestureLabel, int, int]], min_length: int = 5) -> List[Tuple[GestureLabel, int, int]]:
...
def evaluate_and_generate_annotations(
    ...
    sequence_files = sorted([f for f in os.listdir(sequences_folder) if f.endswith(".txt")])
    ...
    for seq_file in sequence_files:
        ...
        # Generate annotation
        annotation = generate_annotations_for_sequence(model, sequence, min_gesture_length)
        annotations.append(annotation)
        
        print(f"Sequence {sequence.sequence_id}: {len(annotation.gestures)} gestures detected")
...
def main():
    ...
    if args.compare_gt:
        ...
        if os.path.exists(gt_file):
            ...
            all_gestures = set(stats['gt_gesture_counts'].keys()) | set(stats['gen_gesture_counts'].keys())
